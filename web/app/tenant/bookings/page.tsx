...
import { TenantShortletBookingsPanel } from "@/components/tenant/TenantShortletBookingsPanel";
...
export default async function TenantBookingsPage() {
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-8">
        <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
          Shortlet bookings are unavailable right now.
        </div>
      </div>
    );
  }
  ...
  if (!user) {
    redirect("/auth/required?redirect=/tenant/bookings&reason=auth");
  }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4 px-4 py-4">
      <TenantShortletBookingsPanel initialRows={bookings} />
    </div>
  );
}
