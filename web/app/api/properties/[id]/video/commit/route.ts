...
export async function POST(
  ...
  const auth = await requireUser({
    request,
    route: `/api/properties/${id}/video/commit`,
    startTime: Date.now(),
    supabase,
  });
  ...
  if (!path || !path.startsWith(`${id}/`)) {
    return NextResponse.json({ error: "Invalid video path.", code: "invalid_path" }, { status: 400 });
  }
  if (!format || !isAllowedVideoType(format)) {
  ...
  const upsert = await supabase
    ...
    .select("video_url, storage_path, bytes, format")
    .maybeSingle();

  if (upsert.error) {
    return NextResponse.json({ error: upsert.error.message, code: "commit_failed" }, { status: 400 });
  }

  return NextResponse.json({
    video_url: upsert.data?.video_url ?? videoUrl,
    storage_path: upsert.data?.storage_path ?? path,
    bytes,
    format,
  });
}
