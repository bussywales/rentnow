...
export async function GET(request: Request) {
  ...
  try {
    const { data, error } = await auth.supabase
      ...
      .select(
        "id, status, preferred_times, approved_time, proposed_times, host_message, decline_reason_code, created_at, decided_at, no_show_reported_at, properties:properties!inner(id, title, city, neighbourhood, timezone)"
      )
