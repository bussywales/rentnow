...
export async function POST(request: Request) {
  ...
  const reference = parsed.data.tx_ref?.trim();
  const transactionId = parsed.data.transaction_id ? String(parsed.data.transaction_id) : null;
  ...
  try {
    let verifyUrl = "";
    if (transactionId) {
      verifyUrl = `https://api.flutterwave.com/v3/transactions/${encodeURIComponent(transactionId)}/verify`;
    } else {
    ...
    if (!verifyRes.ok || payload?.status !== "success") {
      await adminDb
        .from("provider_payment_events")
      ...
      return NextResponse.json(
        { error: payload?.message || "Flutterwave verification failed." },
        { status: 502 }
      );
    }

    const transactionStatus = payload?.data?.status;
    const resolvedTransactionId = payload?.data?.id ? String(payload.data.id) : transactionId;
