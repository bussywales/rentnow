...
CREATE INDEX idx_properties_featured_control ON public.properties (is_featured, featured_rank, featured_until);
CREATE INDEX idx_properties_featured_live ON public.properties (featured_rank ASC, updated_at DESC)
...
CREATE TABLE public.listing_transfer_requests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  property_id UUID NOT NULL REFERENCES public.properties (id) ON DELETE CASCADE,
...
CREATE INDEX idx_listing_transfer_requests_property ON public.listing_transfer_requests (property_id, created_at DESC);
CREATE INDEX idx_listing_transfer_requests_from_owner ON public.listing_transfer_requests (from_owner_id, created_at DESC);
CREATE INDEX idx_listing_transfer_requests_to_owner ON public.listing_transfer_requests (to_owner_id, created_at DESC);
CREATE INDEX idx_listing_transfer_requests_status ON public.listing_transfer_requests (status, created_at DESC);
CREATE UNIQUE INDEX idx_listing_transfer_requests_pending_property
  ...
  IF thread_conflict_count > 0 THEN
    UPDATE public.listing_transfer_requests
      SET last_failure_code = 'MESSAGE_THREAD_CONFLICT',
          last_failure_reason = 'Recipient already has listing message threads. Resolve the conflict before retrying.',
          last_failed_at = now_ts,
          updated_at = now_ts
      WHERE id = transfer_row.id;
...
-- Subscriptions + featured credits + feature purchases.
