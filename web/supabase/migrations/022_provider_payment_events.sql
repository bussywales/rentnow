-- Provider payment events audit table (idempotent).
...
CREATE TABLE IF NOT EXISTS public.provider_payment_events (
  ...
  event_type TEXT NOT NULL DEFAULT 'initialize',
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'provider_payment_events_provider_check'
      AND conrelid = 'public.provider_payment_events'::regclass
  ) THEN
    ALTER TABLE public.provider_payment_events
      ADD CONSTRAINT provider_payment_events_provider_check
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'provider_payment_events_mode_check'
      AND conrelid = 'public.provider_payment_events'::regclass
  ) THEN
    ALTER TABLE public.provider_payment_events
      ADD CONSTRAINT provider_payment_events_mode_check
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'provider_payment_events_cadence_check'
      AND conrelid = 'public.provider_payment_events'::regclass
  ) THEN
    ALTER TABLE public.provider_payment_events
      ADD CONSTRAINT provider_payment_events_cadence_check
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'provider_payment_events_status_check'
      AND conrelid = 'public.provider_payment_events'::regclass
  ) THEN
    ALTER TABLE public.provider_payment_events
      ADD CONSTRAINT provider_payment_events_status_check
      CHECK (status IN ('initialized', 'verified', 'failed', 'skipped'));
  END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS idx_provider_payment_events_reference
  ON public.provider_payment_events (provider, reference);

CREATE INDEX IF NOT EXISTS idx_provider_payment_events_profile
  ON public.provider_payment_events (profile_id);

CREATE INDEX IF NOT EXISTS idx_provider_payment_events_status
  ON public.provider_payment_events (status);

CREATE INDEX IF NOT EXISTS idx_provider_payment_events_created
  ON public.provider_payment_events (created_at);

CREATE INDEX IF NOT EXISTS idx_provider_payment_events_mode
  ON public.provider_payment_events (mode);

ALTER TABLE public.provider_payment_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.provider_payment_events FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "provider events select self" ON public.provider_payment_events;
CREATE POLICY "provider events select self" ON public.provider_payment_events
  FOR SELECT
  USING (auth.uid() = profile_id);

DROP POLICY IF EXISTS "provider events admin read" ON public.provider_payment_events;
CREATE POLICY "provider events admin read" ON public.provider_payment_events
  FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'
  ));

DROP POLICY IF EXISTS "provider events insert self" ON public.provider_payment_events;
CREATE POLICY "provider events insert self" ON public.provider_payment_events
  FOR INSERT
  WITH CHECK (auth.uid() = profile_id);

DROP POLICY IF EXISTS "provider events update self" ON public.provider_payment_events;
CREATE POLICY "provider events update self" ON public.provider_payment_events
  FOR UPDATE
  USING (auth.uid() = profile_id)
  WITH CHECK (auth.uid() = profile_id);

DROP POLICY IF EXISTS "provider events admin update" ON public.provider_payment_events;
CREATE POLICY "provider events admin update" ON public.provider_payment_events
  FOR UPDATE
  USING (EXISTS (
    SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'
  ))
...
BEGIN
  ...
  rls := jsonb_build_object(
    ...
    'stripe_webhook_events', jsonb_build_object(
      'enabled', COALESCE((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.stripe_webhook_events')), false),
      'forced', COALESCE((SELECT relforcerowsecurity FROM pg_class WHERE oid = to_regclass('public.stripe_webhook_events')), false)
    ),
    ...
    'provider_payment_events', jsonb_build_object(
      'enabled', COALESCE((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.provider_payment_events')), false),
      'forced', COALESCE((SELECT relforcerowsecurity FROM pg_class WHERE oid = to_regclass('public.provider_payment_events')), false)
    )
  );
  ...
  policies := jsonb_build_object(
    ...
    'stripe_webhook_events', COALESCE(
      (SELECT jsonb_agg(policyname ORDER BY policyname)
       FROM pg_policies
    ...
    'provider_payment_events', COALESCE(
      (SELECT jsonb_agg(policyname ORDER BY policyname)
       FROM pg_policies
       WHERE schemaname = 'public' AND tablename = 'provider_payment_events'),
      '[]'::jsonb
    )
  );
  ...
  columns := jsonb_build_object(
    ...
    'stripe_webhook_events', jsonb_build_object(
      'event_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'event_id'
      ),
      'event_type', EXISTS (
        SELECT 1 FROM information_schema.columns
      ...
      'reason', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'reason'
      ),
      'mode', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'mode'
      ),
      'replay_count', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'replay_count'
      ),
      'last_replay_at', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'last_replay_at'
      ),
      'last_replay_status', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'last_replay_status'
      ),
      'last_replay_reason', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'stripe_webhook_events' AND column_name = 'last_replay_reason'
      )
    ),
    'provider_settings', jsonb_build_object(
      'stripe_mode', EXISTS (
    ...
    'provider_payment_events', jsonb_build_object(
      'provider', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'provider'
      ),
      'mode', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'mode'
      ),
      'reference', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'reference'
      ),
      'status', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'status'
      ),
      'plan_tier', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'plan_tier'
      ),
      'profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'profile_id'
      ),
      'cadence', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'cadence'
      ),
      'amount', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'amount'
      ),
      'currency', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'currency'
      ),
      'transaction_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'transaction_id'
      ),
      'processed_at', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'provider_payment_events' AND column_name = 'processed_at'
      )
    )
  );
