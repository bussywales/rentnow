...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'push_subscriptions_profile_endpoint_unique'
      AND conrelid = 'public.push_subscriptions'::regclass
  ) THEN
    ALTER TABLE public.push_subscriptions
      ADD CONSTRAINT push_subscriptions_profile_endpoint_unique
...
CREATE INDEX IF NOT EXISTS idx_push_subscriptions_created
  ON public.push_subscriptions (created_at);
...
BEGIN
  ...
  rls := jsonb_build_object(
    ...
    'push_subscriptions', jsonb_build_object(
      'enabled', COALESCE((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.push_subscriptions')), false),
      'forced', COALESCE((SELECT relforcerowsecurity FROM pg_class WHERE oid = to_regclass('public.push_subscriptions')), false)
    )
  );
  ...
  columns := jsonb_build_object(
    ...
    'push_subscriptions', jsonb_build_object(
      ...
      'profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'push_subscriptions' AND column_name = 'profile_id'
      ),
      'endpoint', EXISTS (
        SELECT 1 FROM information_schema.columns
