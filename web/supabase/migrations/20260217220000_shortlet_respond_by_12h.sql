-- Introduce explicit respond_by and enforce a 12-hour host response window for request bookings.

alter table public.shortlet_bookings
  add column if not exists respond_by timestamptz;

update public.shortlet_bookings
set respond_by = coalesce(
  respond_by,
  expires_at,
  case
    when status in ('pending', 'pending_payment') then created_at + interval '12 hours'
    else null
  end
)
...
update public.shortlet_bookings
set expires_at = coalesce(expires_at, respond_by)
where status in ('pending', 'pending_payment');

create index if not exists shortlet_bookings_respond_by_idx
  on public.shortlet_bookings (respond_by)
...
begin
  ...
  resolved_expiry := case when resolved_mode = 'request' then now_ts + interval '12 hours' else null end;
  ...
  set status = next_status,
      ...
      respond_by = case
        when next_status = 'pending'
          then coalesce(respond_by, expires_at, created_at + interval '12 hours')
        else null
      end,
      expires_at = case
        when next_status = 'pending'
