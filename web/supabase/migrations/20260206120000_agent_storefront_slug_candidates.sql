-- Allow legacy slug variants that include numeric suffixes (e.g., "-2") to resolve.
...
as $$
  with normalized as (
    select
      lower(btrim(coalesce(input_slug, ''))) as input_slug,
      regexp_replace(lower(btrim(coalesce(input_slug, ''))), '-[0-9]+$', '') as base_slug
  ),
  slug_candidates as (
    select input_slug as slug
    from normalized
    where input_slug <> ''
    union
    select base_slug as slug
    from normalized
    where base_slug <> ''
      and base_slug <> input_slug
  ),
  settings as (
    select
      coalesce(
  ...
  storefront as (
    ...
    where s.slug is not null
      and lower(btrim(s.slug)) in (select slug from slug_candidates)
    limit 1
  ),
  ...
  profile_direct as (
    ...
    where p.agent_slug is not null
      and lower(btrim(p.agent_slug)) in (select slug from slug_candidates)
    limit 1
  ),
  legacy_profile as (
    select
    ...
    where p.agent_slug is not null
      ...
      and regexp_replace(
        ...
        '',
        'g'
      ) in (select slug from slug_candidates)
    limit 1
  ),
  resolved as (
    select
