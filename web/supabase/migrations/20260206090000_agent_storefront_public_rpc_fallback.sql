-- Fallback lookup for agent storefronts when agent_storefronts row is missing.
...
as $$
  ...
  resolved as (
    select
      coalesce(ps.id, pd.id) as id,
      coalesce(ps.role, pd.role) as role,
      coalesce(ps.display_name, pd.display_name) as display_name,
      coalesce(ps.full_name, pd.full_name) as full_name,
      coalesce(ps.business_name, pd.business_name) as business_name,
      coalesce(ps.avatar_url, pd.avatar_url) as avatar_url,
      coalesce(s.slug, ps.agent_slug, pd.agent_slug) as slug,
      coalesce(s.enabled, ps.agent_storefront_enabled, pd.agent_storefront_enabled, true) as enabled,
      coalesce(s.bio, ps.agent_bio, pd.agent_bio) as bio
    from storefront s
