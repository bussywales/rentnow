-- Messaging throttle telemetry (idempotent).

CREATE TABLE IF NOT EXISTS public.messaging_throttle_events (
...
CREATE INDEX IF NOT EXISTS idx_messaging_throttle_events_actor
  ON public.messaging_throttle_events (actor_profile_id);

CREATE INDEX IF NOT EXISTS idx_messaging_throttle_events_created
  ON public.messaging_throttle_events (created_at);

CREATE INDEX IF NOT EXISTS idx_messaging_throttle_events_thread
  ON public.messaging_throttle_events (thread_key);

ALTER TABLE public.messaging_throttle_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.messaging_throttle_events FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "messaging throttle admin read" ON public.messaging_throttle_events;
CREATE POLICY "messaging throttle admin read" ON public.messaging_throttle_events
  FOR SELECT
  USING (
    EXISTS (
...
BEGIN
  ...
  rls := jsonb_build_object(
    ...
    'role_change_audit', jsonb_build_object(
      'enabled', COALESCE((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.role_change_audit')), false),
      'forced', COALESCE((SELECT relforcerowsecurity FROM pg_class WHERE oid = to_regclass('public.role_change_audit')), false)
    ),
    'messaging_throttle_events', jsonb_build_object(
      'enabled', COALESCE((SELECT relrowsecurity FROM pg_class WHERE oid = to_regclass('public.messaging_throttle_events')), false),
      'forced', COALESCE((SELECT relforcerowsecurity FROM pg_class WHERE oid = to_regclass('public.messaging_throttle_events')), false)
    )
  );
  ...
  policies := jsonb_build_object(
    ...
    'role_change_audit', COALESCE(
      (SELECT jsonb_agg(policyname ORDER BY policyname)
       FROM pg_policies
       WHERE schemaname = 'public' AND tablename = 'role_change_audit'),
      '[]'::jsonb
    ),
    'messaging_throttle_events', COALESCE(
      (SELECT jsonb_agg(policyname ORDER BY policyname)
       FROM pg_policies
       WHERE schemaname = 'public' AND tablename = 'messaging_throttle_events'),
      '[]'::jsonb
    )
  );
  ...
  columns := jsonb_build_object(
    ...
    'role_change_audit', jsonb_build_object(
      'target_profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'role_change_audit' AND column_name = 'target_profile_id'
      ),
      'actor_profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'role_change_audit' AND column_name = 'actor_profile_id'
      ),
      'new_role', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'role_change_audit' AND column_name = 'new_role'
      ),
      'created_at', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'role_change_audit' AND column_name = 'created_at'
      )
    ),
    'messaging_throttle_events', jsonb_build_object(
      'id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'id'
      ),
      'created_at', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'created_at'
      ),
      'actor_profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'actor_profile_id'
      ),
      'thread_key', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'thread_key'
      ),
      'property_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'property_id'
      ),
      'recipient_profile_id', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'recipient_profile_id'
      ),
      'reason_code', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'reason_code'
      ),
      'retry_after_seconds', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'retry_after_seconds'
      ),
      'window_seconds', EXISTS (
        SELECT 1 FROM information_schema.columns
      ...
      'mode', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'mode'
      ),
      'ip_hash', EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'messaging_throttle_events' AND column_name = 'ip_hash'
      )
    )
  );
