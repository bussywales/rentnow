-- Role defaults + admin role change audit (idempotent).
...
SET role = 'tenant'
WHERE role IS NULL
  OR role::text NOT IN ('tenant', 'landlord', 'agent', 'admin');
...
CREATE INDEX IF NOT EXISTS idx_role_change_audit_created
  ON public.role_change_audit (created_at);

ALTER TABLE public.role_change_audit ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.role_change_audit FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "role change audit admin read" ON public.role_change_audit;
CREATE POLICY "role change audit admin read" ON public.role_change_audit
  FOR SELECT
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "role change audit admin insert" ON public.role_change_audit;
CREATE POLICY "role change audit admin insert" ON public.role_change_audit
  FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.profiles p
