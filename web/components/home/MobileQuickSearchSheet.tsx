...
import { writeStoredIntentForMarket, type MobileQuickSearchIntent } from "@/lib/home/mobile-quicksearch-intent";
import {
...
export function MobileQuickSearchSheet({ open, onOpenChange, sheetId }: MobileQuickSearchSheetProps) {
  ...
  const initialState = useMemo(() => {
    const defaultCategory: MobileQuickSearchCategory = "all";
    if (typeof window === "undefined") {
      return { category: defaultCategory, city: "" };
    }
    const lastSearchParams = null;
    return {
  ...
  return (
    ...
    >
      ...
      >
        <div className="space-y-1.5" data-testid="mobile-quicksearch-category-rail">
          ...
          {MOBILE_QUICKSEARCH_CATEGORY_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 snap-start rounded-full border px-3 py-1.5 text-xs font-semibold transition ${
                  active
        ...
        {presetOptions.length > 0 ? (
          <div className="space-y-1.5" data-testid="mobile-quicksearch-presets">
            <div className="scrollbar-none -mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-1">
        ...
        <div className="space-y-1.5" data-testid="mobile-quicksearch-dates">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Dates & guests</p>
            <p className="text-[11px] text-slate-500" data-testid="mobile-quicksearch-date-summary">
              {dateSummary} · {guestsDraft} guest{guestsDraft === 1 ? "" : "s"}
            </p>
          </div>
          <div className="flex items-center justify-between gap-2">
            <div className="scrollbar-none -mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 py-0.5">
            ...
            <div className="flex items-center gap-1.5" data-testid="mobile-quicksearch-guests">
              <button
                type="button"
                disabled={!isShortletIntent || guestsDraft <= 1}
                data-testid="mobile-quicksearch-guests-decrement"
                className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-white text-base font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-60"
                onClick={() => setGuestsDraft((value) => normalizeGuests(value - 1))}
              >
                -
              </button>
              <p className="min-w-5 text-center text-sm font-semibold text-slate-900" data-testid="mobile-quicksearch-guests-value">
                {guestsDraft}
              </p>
              <button
                type="button"
        ...
        {mergedRecentItems.length > 0 ? (
          <div className="space-y-1.5" data-testid="mobile-quicksearch-recents">
            <div className="flex items-center justify-between gap-2">
            ...
            <div className="scrollbar-none -mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1 pb-1">
              {mergedRecentItems.map((recent) => (
                <button
                  ...
                  className="shrink-0 snap-start rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700"
                  onClick={() => {
                    if (recent.query) {
          ...
          <div className="space-y-1.5" data-testid="mobile-quicksearch-empty-suggestions">
        ...
        <div className={`grid gap-2 ${lastSearchHref ? "grid-cols-2" : "grid-cols-1"}`}>
          {lastSearchHref ? (
            <button
              type="button"
