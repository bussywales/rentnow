...
test.describe.serial("lead attribution from client page", () => {
  test("agent sees client page attribution in inbox", async ({ page, context }) => {
    ...
    try {
      ...
      await page.getByTestId("client-page-name").fill("Buyer Concierge");
      ...
      await tenantPage.getByText("Client page lead").click();
      ...
      await expect(page.getByText(/Buyer Concierge/i)).toBeVisible();
      await expect(page.getByText(/Open client page/i)).toBeVisible();
    } finally {
      if (leadId) {
