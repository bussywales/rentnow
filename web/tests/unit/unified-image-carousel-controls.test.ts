...
void test("unified image carousel consumes the shared interaction policy module", () => {
  ...
  assert.ok(contents.includes('from "@/components/layout/ImageOptimizationModeProvider"'));
  assert.ok(contents.includes('from "@/lib/media/image-optimization-mode"'));
  ...
  assert.ok(contents.includes("loader={unoptimized ? directImageLoader : undefined}"));
