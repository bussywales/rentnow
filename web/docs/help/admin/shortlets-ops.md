...
- Checkout now supports Stripe and Paystack for shortlet bookings.
- Manual payouts remain the active settlement model for shortlets.

## What to monitor

- Payment webhook processing (`/api/webhooks/stripe`, `/api/webhooks/paystack`) and idempotent replays.
...
Do not promise automated payout behavior in this pilot. Keep `shortlet_auto_payouts_enabled` set to false unless a formal rollout is approved.
</Callout>
