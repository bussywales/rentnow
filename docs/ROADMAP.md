...
- vR16.9b.15: Admin review OR clause + service schema/url fix with explicit pending statuses, raw PostgREST ping diagnostics, and service-role preference/fallback warnings.
...
- **R16.9b.3 — Admin review saved views/filters (Shipped)**: /admin/review gains saved views (Pending/Changes/Approved/All), URL/localStorage persistence, search + filters (video/location/photos), deterministic sorting, and drawer prev/next + hidden-by-filters handling; still single-item actions only.
- **R16.9b.4 — Admin review rubric (Shipped)**: request-changes now uses structured reason codes + auto-generated editable messages stored as JSON in rejection_reason; drawer shows preview/regenerate controls with locked microcopy and keeps legacy text-compatible parsing.
- **R16.9b.5 — Host fix requests panel (Shipped)**: host editor shows a “Fix requested” panel for changes_requested listings, mapping admin rubric reasons to host-friendly labels with deep links to Photos/Location and session-only dismissal; legacy rejection reasons still supported.
...
- **R16.9b.10 — Admin review queue visibility (Shipped)**: Canonical reviewable predicate (pending/submitted + approval flags) shared across badge, Review Desk, and diagnostics; service-role fallback for admins; warning when service key missing; richer diagnostics and guardrail tests.
- **R16.9b.12 — Admin review queue service (Shipped)**: service-role queue fetch is the source of truth for admins, reviewable predicate/or-clause tightened, diagnostics returns user/service counts and errors clearly, and both badge and Review Desk stay in sync.
- **R16.9b.13 — Admin review service URL fix (Shipped)**: normalized Supabase URL for service client to avoid 404s; service-role fetch with user fallback and diagnostics clarity on URL/host and service errors.
